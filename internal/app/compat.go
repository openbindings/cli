...
type OperationReport struct {
	...
	// KindMatch is true if the candidate's kind matches the target's kind.
	// Only meaningful when Matched is true.
	KindMatch bool `json:"kindMatch,omitempty"`

	// Per-slot status: compatible, incompatible, or unspecified.
	Input   SlotStatus `json:"input,omitempty"`   // method only
	Output  SlotStatus `json:"output,omitempty"`  // method only
	Payload SlotStatus `json:"payload,omitempty"` // event only
...
func CompatibilityCheck(input CompatInput) CompatibilityReport {
	...
	// Build the report.
	// Pass target locator so satisfies/imports matching can resolve references.
...
// so that satisfies/imports matching can resolve references.
...
//     satisfies: [{ interface: <importKey>, operation: <opOrAlias> }]
//     where candidate.imports[importKey] resolves to targetLocator,
...
func matchOperation(name string, tgtOp openbindings.Operation, targetLocator string, candidate *openbindings.Interface) (openbindings.Operation, bool) {
	...
	if candidate.Imports != nil && targetLocator != "" {
		// Build a set of import keys that resolve to the target.
		targetImportKeys := make(map[string]bool)
		for key, loc := range candidate.Imports {
			if loc == targetLocator {
				targetImportKeys[key] = true
			}
		}

		if len(targetImportKeys) > 0 {
			...
			for _, candOp := range candidate.Operations {
				for _, sat := range candOp.Satisfies {
					if targetImportKeys[sat.Interface] && targetNames[sat.Operation] {
						return candOp, true
					}
				}
			}
...
func buildOperationReport(
	...
	report := OperationReport{
		...
		Kind:      string(tgtOp.Kind),
		Matched:   true,
		KindMatch: tgtOp.Kind == candOp.Kind,
	}
	...
	switch tgtOp.Kind {
	case openbindings.OperationKindMethod:
		var detail string
	...
	case openbindings.OperationKindEvent:
		var detail string
		report.Payload, detail = slotCompat("payload", tgtOp.Payload, candOp.Payload, tgtNorm, candNorm, false)
		if detail != "" {
			details = append(details, detail)
		}
	}
	...
	if !report.KindMatch {
		details = append(details, fmt.Sprintf("kind mismatch: expected %s, got %s", tgtOp.Kind, candOp.Kind))
	}

	report.Details = details

	// An operation is compatible if:
	// - kind matches
	// - no slot is incompatible (unspecified is ok per spec)
	report.Compatible = report.KindMatch &&
		report.Input != SlotIncompatible &&
		report.Output != SlotIncompatible &&
		report.Payload != SlotIncompatible
...
func slotCompat(
	...
	var ok bool
	if isInput {
		ok, err = n.InputCompatible(tgtNormalized, candNormalized)
	} else {
		ok, err = n.OutputCompatible(tgtNormalized, candNormalized)
	}
	if err != nil {
...
func (r CompatibilityReport) Render() string {
	...
	for _, op := range r.Operations {
		...
		sb.WriteString(s.Dim.Render(" (" + op.Kind + ")"))
