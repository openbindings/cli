...
func TestCompatibilityCheck_IdenticalInterfaces(t *testing.T) {
	...
	if !op.KindMatch {
		t.Error("expected kind to match")
	}
...
func TestCompatibilityCheck_KindMismatch(t *testing.T) {
	dir := t.TempDir()
	...
	if report.Compatible {
		t.Error("expected incompatible (kind mismatch)")
	}
	...
	if op.Compatible {
		t.Error("expected operation to be incompatible")
	}
	if len(op.Details) == 0 {
		t.Error("expected details about kind mismatch")
	}
}
...
func TestCompatibilityCheck_EventPayload(t *testing.T) {
	dir := t.TempDir()
	...
	if !report.Compatible {
		t.Error("expected compatible event")
	}
	op := report.Operations[0]
	if op.Payload != SlotCompatible {
		t.Errorf("expected payload=compatible, got %q", op.Payload)
	}
	// Input/output should not be set for events.
	if op.Input != "" {
		t.Errorf("expected input empty for event, got %q", op.Input)
	}
	if op.Output != "" {
		t.Errorf("expected output empty for event, got %q", op.Output)
	}
}
...
// TestCompatibilityCheck_SatisfiesMatch verifies that satisfies/imports
...
// TestCompatibilityCheck_SatisfiesNoImportFallsBack verifies that when
// satisfies references an import key that doesn't match the target,
// the algorithm falls back to key/alias matching.
func TestCompatibilityCheck_SatisfiesNoImportFallsBack(t *testing.T) {
	dir := t.TempDir()
