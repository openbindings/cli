...
func TestDetectCrossSourceDrift_KindDiffers(t *testing.T) {
	perSource := []perSourceDerivation{
		{
			key:    "sourceA",
			format: "usage@2.0.0",
	...
	if drift[0].Operation != "notify" {
		t.Errorf("expected 'notify', got %q", drift[0].Operation)
	}
}
