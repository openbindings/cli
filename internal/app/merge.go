...
func applyMerge(target, source *openbindings.Interface, entries []MergeEntry) {
	for _, e := range entries {
		...
		case MergeAdd:
			...
			// Add corresponding bindings from source.
		...
		case MergeUpdate:
			...
			if sourceOp, ok := source.Operations[e.Operation]; ok {
				...
				targetOp.Payload = sourceOp.Payload

				// Replace kind if changed.
				if sourceOp.Kind != "" {
					targetOp.Kind = sourceOp.Kind
				}
...
func migrateSchemaRefs(target, source *openbindings.Interface, op openbindings.Operation) {
	...
	refs = append(refs, collectRefs(op.Payload)...)
