...
// FormatOutputString is a convenience function that returns a string.
// For text format, returns empty string (caller should handle text output).
func FormatOutputString(format OutputFormat, data any) (string, error) {
	if format == OutputFormatText {
		return "", nil
	}
	b, err := FormatOutput(data, format)
	if err != nil {
		return "", err
	}
	return string(b), nil
