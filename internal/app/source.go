...
type SourceRemoveOutput struct {
	Key              string `json:"key"`
	OrphanedBindings int    `json:"orphanedBindings,omitempty"`
}
...
func (o SourceRemoveOutput) Render() string {
	...
	sb.WriteString(" ")
	sb.WriteString(s.Key.Render(o.Key))
	if o.OrphanedBindings > 0 {
		sb.WriteString("\n")
		sb.WriteString(s.Warning.Render(fmt.Sprintf(
			"  warning: %d binding(s) still reference this source",
			o.OrphanedBindings,
		)))
	}
	return sb.String()
...
// SourceRemove removes a source reference from an OBI file.
// It warns about orphaned bindings but does not remove them.
