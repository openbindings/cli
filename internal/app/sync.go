...
func Sync(input SyncInput) (SyncOutput, error) {
	...
	// preDiscovered holds results from SourceDiscoverer delegates, keyed by source key.
	...
	for _, key := range targetKeys {
		...
		// Check if the handler for this format implements SourceDiscoverer.
		// Runtime-discovery sources (e.g., MCP servers) need this path because
		// they require a protocol handshake — ReadAndHashSource would hang.
		handler, handlerErr := DefaultRegistry().ForFormat(src.Format)
		if handlerErr == nil {
			if discoverer, ok := handler.(delegates.SourceDiscoverer); ok {
				discoverCtx, discoverCancel := context.WithTimeout(context.Background(), 60*time.Second)
				data, derived, discoverErr := discoverer.DiscoverSource(
					discoverCtx,
					delegates.Source{Format: src.Format, Location: meta.Ref},
				)
				discoverCancel()
				if discoverErr != nil {
				...
				// Remap binding source keys to the actual source key.
	...
	// For each synced source, re-derive via the handler and merge against the OBI.
	...
	for _, key := range syncedKeys {
		...
		if pre, ok := preDiscovered[key]; ok {
			// Use the pre-discovered result (SourceDiscoverer path).
			derived = pre
		} else {
			// Default path: re-derive via the handler.
			var err error
...
// remapBindings converts an openbindings.Interface from a SourceDiscoverer
...
func remapBindings(iface openbindings.Interface, sourceKey string) DeriveResult {
	bindings := make(map[string]openbindings.BindingEntry, len(iface.Bindings))
