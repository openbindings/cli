...
func TestValidateInterface_InvalidOperationKind(t *testing.T) {
	dir := t.TempDir()
	path := writeInterface(t, dir, "bad-kind.json", map[string]any{
		"openbindings": "0.1.0",
	...
	if len(report.Problems) == 0 {
		t.Fatal("expected problems about invalid kind")
	}
}
...
func TestValidateInterface_DuplicateAliases(t *testing.T) {
	...
	path := writeInterface(t, dir, "dup-alias.json", map[string]any{
		"openbindings": "0.1.0",
		"operations": map[string]any{
			"a": map[string]any{"kind": "method", "aliases": []any{"shared"}},
			"b": map[string]any{"kind": "method", "aliases": []any{"shared"}},
		},
	})
