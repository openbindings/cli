...
// matchTier represents preference specificity (higher = more specific).
// Uses iota for inherent ordering - adding a tier just requires inserting
// a line in the correct position.
type matchTier int

const (
	matchTierNone       matchTier = iota // No match
	matchTierNameOnly                    // "usage" (catch-all)
	matchTierOther                       // "usage@>=2.0.0" (broad constraint)
	matchTierTilde                       // "usage@~2.1.0" (minor-specific)
	matchTierCaret                       // "usage@^2.0.0" (major-specific)
	matchTierExact                       // "usage@2.0.0" (exact version)
	matchTierIdentical                   // Exact string match
)

// matchTierFor returns the specificity tier of a preference key for a requested format.
// Higher tiers are more specific and take precedence during delegate resolution.
func matchTierFor(prefKey, requested string) matchTier {
	if strings.EqualFold(strings.TrimSpace(prefKey), strings.TrimSpace(requested)) {
		return matchTierIdentical
	}
	_, v := parseFormatToken(prefKey)
	v = strings.TrimSpace(v)
	if v == "" {
		return matchTierNameOnly
	}
	if isExactVersionString(v) {
		return matchTierExact
	}
	if strings.HasPrefix(v, "^") {
		return matchTierCaret
	}
	if strings.HasPrefix(v, "~") {
		return matchTierTilde
	}
	return matchTierOther
}
